//this was breaking stuff when on pages that didn't have bookable resources!! need to fix!!
...
var calendar = function(){
    ...
    var current_resource = function(){
    	return window.location.href.match(/karaoke_rooms\/(\d+)\/bookings/)[1];
    };
    ...
    $('#calendar').fullCalendar({
        header: {
			...
			eventSources: [{  
    		url: '/karaoke_rooms/'+current_resource()+'/bookings/',  
   		}],
    ...
    eventDrop: function(booking) {
      var length = (booking.end-booking.start)/(3600000);
        ...
        function updateEvent(booking) {
              $.ajax(
                '/karaoke_rooms/'+current_resource()+'/bookings/'+booking.id,
                { 'type': 'PATCH',
    ...
    ,
      ...
      if (window.location.href.match(/new/)) {
        if(today_or_later()) {
          ...
          jQuery.post(
            '/karaoke_rooms/'+current_resource()+'/bookings',
